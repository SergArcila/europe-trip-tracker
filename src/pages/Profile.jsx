...
export default function Profile() {
  ...
  const [homeCountry, setHomeCountry] = useState(null); // { name, flag, code }
  const [countriesVisited, setCountriesVisited] = useState([]); // [{ name, flag, code }]
  ...
  const handleRemoveVisited = (name) => {
    const next = countriesVisited.filter(c => c.name !== name);
    setCountriesVisited(next);
    saveCountries(homeCountry, next);
  };
  ...
  // Exclude home country and already-visited from the "visited" picker
